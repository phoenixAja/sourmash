...
class KmerMinHash
{
  ...
  public:
    KmerMinHash(unsigned int n, unsigned int k, bool prot, bool dayhoff, uint32_t s,
                HashIntoType mx) {
      _this = kmerminhash_new(n, k, prot, dayhoff, s, mx, false);
    };
...
class KmerMinAbundance: public KmerMinHash {
 public:
    KmerMinAbundance(unsigned int n, unsigned int k, bool prot, bool dayhoff, uint32_t seed,
                     HashIntoType mx) : KmerMinHash(n, k, prot, dayhoff, seed, mx) {
      kmerminhash_free(_this);
      _this = kmerminhash_new(n, k, prot, dayhoff, seed, mx, true);
    };
